...
import { API_TYPE, Copy, HTTPUtils, HTTP_ENCODING_TYPE, HTTP_METHOD, LocalisationUtils, UseAllLookups, UseUserResponse } from '@/lib'
import { useEffect, useId, useState } from 'react'
import { Form, FormElement, Field, FieldWrapper } from "@progress/kendo-react-form"
import { IChangePasswordRequest, ISuperConstants, TypeUtility } from '@david.ezechukwu/core'
...
export default function ResetPasswordPage() {    
    ...
    const [_, UserResponse, SetUserResponse] = UseUserResponse()
    ...
    const PasswordFieldName = TypeUtility.NameOf<IChangePasswordRequest>('Password')
    const PasswordConfirmationFieldName = TypeUtility.NameOf<IChangePasswordRequest>('PasswordConfirmation')
    ...
    const OnSubmit = async (formData: any) => {
        try {
            ...
            const reqBodyObject: IChangePasswordRequest = {
                Password: PasswordValue,
                PasswordConfirmation: PasswordConfirmationValue,
                PasswordStrength: passwordStrength,
                Locale: locale ?? LocalisationUtils.DefaultLocale
            }
            const urlPath = `user/${UserResponse?.UserId}/password?locale=${locale ?? LocalisationUtils.DefaultLocale}`
            ...
            const succeeded = await HTTPUtils.RequestUsingPromise(API_TYPE.CORE, HTTP_METHOD.POST, HTTP_ENCODING_TYPE.FORM_ENCODED, urlPath, reqBodyObject, requestOptions)
            SetIsProcessing(false)
            SetPasswordChanged(succeeded)
            if (!succeeded) {
    ...
    return (
        <section>
            <h1>{copy.Pages.User.ChangePassword.MainHeading}</h1>
            <div className={`${styles.pkchangepasswordpagemaincontent}`}>
                {IsProcessing &&
                        ...
                        <FormElement className={`${styles.pkform} ${styles.pkloginform}`}>
                            <fieldset className={'k-form-fieldset'}>
                                ...
                                <Field
                                    ...
                                    name={TypeUtility.NameOf<IChangePasswordRequest>('PasswordStrength')}
                                ...
                                <Field
                                    ...
                                    name={TypeUtility.NameOf<IChangePasswordRequest>('Password')}
                                ...
                                <Field
                                    ...
                                    name={TypeUtility.NameOf<IChangePasswordRequest>('PasswordConfirmation')}
                ...
                {PasswordChanged &&                     
                    <>
                        <mark>{copy.Pages.User.ChangePassword.PasswordChanged}</mark>
                        <br/>
                        <br/>                        
